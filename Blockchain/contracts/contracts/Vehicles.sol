...
contract Vehicles {
    ...
    function renewVehicle(string memory vin) public {
        uint newdate = block.timestamp + 2*365 days;
        for (uint i = 0; i < vehicles[msg.sender].length; i++) {
            if (StringUtils.compare(vehicles[msg.sender][i].vin, vin) == 0) {
                VehicleData memory vehicle = vehicles[msg.sender][i];
                vehicle.RegExpdate = newdate;
                vehicles[msg.sender][i] = vehicle;
                }
                break;
            }
        }
